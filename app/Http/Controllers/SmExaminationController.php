...
//            dd($e);
    ...
    public function make_merit_list($InputClassId, $InputSectionId, $InputExamId, Request $request)
    {
        ...
        foreach ($eligible_students as $SingleStudent) {
            ...
            if ($is_absent == "") {
                $insert_results->result             = optional($markGrades)->grade_name;
            } else {
...
//        try{
        ...
        //    dd( $is_data);
            ...
            foreach ($eligible_students as $SingleStudent) {
                ...
                $markGrades = SmMarksGrade::where([['percent_from', '<=', $average_mark], ['percent_upto', '>=', $average_mark]])->where('school_id',Auth::user()->school_id)->first();
...
//        }
    ...
    public function meritListPrint($exam_id, $class_id, $section_id)
    {
       ...
       return view('backEnd.reports.merit_list_report_print',compact('exams','classes','subjects','class','section','exam','subjectlist','allresult_data','class_name','assign_subjects','exam_name','optional_subject_setup','allresult_dat','total_other','total_female','total_male','totals'));
    ...
    public function markSheetReportStudent(Request $request)
    {
        try{
            ...
            return view('backEnd.reports.mark_sheet_report_student', compact('exams', 'classes','students'));
        }catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back(); 
        }
