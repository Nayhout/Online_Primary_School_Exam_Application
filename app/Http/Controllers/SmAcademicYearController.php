...
class SmAcademicYearController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($input, [
            'year' => 'required|unique:sm_academic_years,year',
        ...
        try {
            ...
            foreach ($tables as $table_name) {
                $data = $table_name::where('created_at', 'like', '%' . $created_year . '%')->get();
                if (!empty($data)) {
...
//            $academic_year->starting_date = $request->year . '-08-01';
            $academic_year->ending_date = date('Y-m-d', strtotime($request->ending_date));
//            $academic_year->ending_date = $request->year+1 . '-07-31';
