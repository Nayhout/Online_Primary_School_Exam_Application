...
class SmOptionalSubjectAssignController extends Controller
{
    ...
    public function assignOptionalSubjectSearch(Request $request)
    {
        ...
        try{
            ...
            $classes = SmClass::join('sm_class_optional_subject','sm_class_optional_subject.class_id','=','sm_classes.id')->where('sm_classes.created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->select('sm_classes.*')->where('sm_class_optional_subject.school_id',Auth::user()->school_id)->get();
            //return $classes;
