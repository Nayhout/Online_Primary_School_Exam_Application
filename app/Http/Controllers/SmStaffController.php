    ...
    public function staffStore(Request $request)
    {
        ...
        try{
            ...
            try {
                ...
                $current_address = "";
                if ($request->file('current_address') != "") {
                    $file = $request->file('current_address');
                    $current_address = md5($file->getClientOriginalName() . time()) . "." . $file->getClientOriginalExtension();
                    $file->move('public/uploads/address/', $current_address);
                    $current_address = 'public/uploads/address/' . $current_address;
                }
                ...
                $staff->current_address = $current_address;
//                if (@Session::get('current_address')) {
//                    $staff->current_address = Session::get('current_address');
//                }
                ...
                $staff->identity = $request->identity;
                ...
                $staff->telegram_url = $request->telegram_url;
 ...
 */
        ...
        try{
            ...
            if ($request->file('current_address') != "") {
                $addressExistFile = SmStaff::find($request->staff_id);
                if ($addressExistFile->current_address != '' && file_exists($addressExistFile->current_address)) {
                    unlink($addressExistFile->current_address);
                }
                $file = $request->file('current_address');
                $current_address = md5($file->getClientOriginalName() . time()) . "." . $file->getClientOriginalExtension();
                $file->move('public/uploads/address/', $current_address);
                ...
                $addressExistFile = SmStaff::find($request->staff_id);
                $current_address = $addressExistFile->current_address;
            }
    ...
    public function viewStaff($id)
    {
        ...
        try{
            $staffDetails = SmStaff::find($id);
             if(Auth::user()->role_id!=1 && Auth::user()->role_id!=5 && Auth::user()->staff->id!=$id){
...
//    }
    ...
    public function staffCurrentSearch(Request $request)
    {
//        dd($request->all());
        $input = $request->all();
        $validator = Validator::make($input, [
...
//        try {
        ...
        $UpDepartment = SmHumanDepartment::find($current_department);

        $UpDesignation = SmDesignation::find($current_designation);

//        if (@$staffs['allresult_data'] ? $staffs['allresult_data']->isEmpty() : empty($staffs)) {
...
//            dd($data);
            ...
            return view('backEnd.humanResource.staff_promote', compact('departments','designations','UpDepartment','UpDesignation','current_designation','current_department'));
...
//        }
    ...
    public function staffCurrentSearchCustom(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
...
//    }
    public function staffPromoteStore(Request $request)
    {
//dd($request->all());
        $input = $request->all();
        $validator = Validator::make($input, [
