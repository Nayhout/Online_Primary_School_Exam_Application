...
use App\StudentAttendanceDetail;
use App\YearCheck;
...
class StudentAttendanceDetailController extends Controller
{
    ...
    public function studentAttendanceDetail(Request $request)
    {
        try {
            ...
            return view('backEnd.studentInformation.student_attendance_detail', compact('classes', 'types', 'genders'));
        } catch (\Exception $e) {
    ...
    public function studentSearchAttType(Request $request)
    {
//        dd($request->all());
        try {
...
//            dd($att_type);
            ...
            $obj = StudentAttendanceDetail::join('sm_students','sm_students.id','sm_student_attendances.student_id')
            ->where(function ($query) use ($att_type) {
                if ($att_type != null) {

                    return $query->where('sm_student_attendances.attendance_type',$att_type);

                }
            })
                ->where(function ($query) use ($class) {
                    if ($class != null) {
                        return $query->where('sm_students.class_id', $class);
                ...
                ->where(function ($query) use ($section) {
                    if ($section != null) {
                        ...
                        return $query->where('sm_students.section_id', $section);
                    }
                })
                ->where(function ($query) use ($date) {
                    if ($date != null) {

                        return $query->whereDate('sm_student_attendances.attendance_date', Carbon::parse($date)->format('Y-m-d'));

                    }
                })
                ->select('sm_student_attendances.*')
                ->get();
            //dd($obj);
            $classes = SmClass::where('active_status', 1)->where('created_at', 'LIKE', '%' . YearCheck::getYear() . '%')->where('school_id', Auth::user()->school_id)->get();
            $attendances = [];
            return view('backEnd.studentInformation.student_attendance_detail_report',[
                'rows' => $obj,
                'classes' => $classes,
            ...
            try {
                ...
                if (!empty($data)) {
                    ...
                    foreach (array_unique($class_sections) as $value) {
                        ...
                        foreach ($students as $student) {
                            SmStudentAttendance::where('student_id', $student->id)->where('attendance_date', date('Y-m-d', strtotime($request->attendance_date)))->delete();
