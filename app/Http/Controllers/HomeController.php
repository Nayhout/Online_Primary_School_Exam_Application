...
//            dd($totalStudents);
            ...
            $toDoLists = SmToDo::where('complete_status','<>','Completed')->where('created_by', $user_id)->where('school_id', Auth::user()->school_id)->get();
            $toDoListsCompleteds = SmToDo::where('complete_status', 'Completed')->where('created_by', $user_id)->where('school_id', Auth::user()->school_id)->get();
            ...
            // return $holidays;
            $assigns = SmStaff::where('active_status',1)->where('school_id', Auth::user()->school_id)->get();
            ...
            return view('backEnd.dashboard', compact('currency', 'totalStudents', 'totalTeachers', 'totalParents', 'totalStaffs', 'toDoLists', 'notices', 'toDoListsCompleteds', 'm_total_income', 'm_total_expense', 'y_total_income', 'y_total_expense', 'holidays', 'year', 'events','assigns'));
        } catch (\Exception $e) {
            // dd($e->getMessage());
            Auth::logout();
            session(['role_id' => '']);
...
//        try {
            ...
            $toDolists->todo_title = $request->name;
            $toDolists->complete_status = $request->complete_status;
            $toDolists->assign_staff_id = $request->assign_staff_id;
            $toDolists->description = $request->description;
            $toDolists->date = date('Y-m-d', strtotime($request->date));
            $toDolists->due_date = date('Y-m-d', strtotime($request->due_date));
...
//    }
    ...
    public function removeToDo(Request $request)
    {
        ...
        try {
            ...
            $to_do->complete_status = "Completed";
            $to_do->save();
            $html = "";
    ...
    public function getToDoList(Request $request)
    {
        try {
            ...
            $to_do_list = SmToDo::where('complete_status', 'Completed')->where('school_id', Auth::user()->school_id)->get();
            $datas = [];
...
//    }
    ...
    {
        $rapport = SmStaff::findOrFail($id);
//        dd($rapport);
        $file = "./uploads/".$rapport;
//        dd($file);
        return Response::download($file);
    }
    ...
    public function ToDoLists(Request $request){
        try{
            $assigns = SmToDo::where('active_status',1)->where('created_at','Like','%'.YearCheck::getYear().'%')->get();
            $assigns_to = SmStaff::where('active_status',1)->get();
            $totalNotStarts = SmToDo::where('active_status', 1)->where('complete_status','Not Started')->where('school_id', Auth::user()->school_id)->get();
            $totalPending = SmToDo::where('active_status', 1)->where('complete_status','Pending')->where('school_id', Auth::user()->school_id)->get();
            $totalAwaits = SmToDo::where('active_status', 1)->where('complete_status','Await Feedback')->where('school_id', Auth::user()->school_id)->get();
            $totalCompletes = SmToDo::where('active_status', 1)->where('complete_status','Completed')->where('school_id', Auth::user()->school_id)->get();
            $totalTests = SmToDo::where('active_status', 1)->where('complete_status','Testing')->where('school_id', Auth::user()->school_id)->get();
        }catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }

        return view('backEnd.communicate.to_do_list', compact('assigns','assigns_to','totalNotStarts','totalAwaits','totalCompletes','totalPending','totalTests'));
    }
    public function deleteToDoLists(Request $request, $id)
    {
        try {
            ...
            return view('backEnd.communicate.delete_todo_list', compact('id'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function deleteToDo(Request $request, $id)
    {
        ...
        try {
            $result = SmToDo::destroy($id);

            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                if ($result) {
                    return ApiBaseMethod::sendResponse(null, 'Task has been deleted successfully');
                } else {
...
//            if ($request->id) {
        ...
        $type = $value[1];
         ...
         $data = SmToDo::find($id);
         $data->complete_status = $type;
//        return ['hehe'=>$id,'hehe2'=>$type];
//                $data = SmAcademicYear::find($request->id);
              //  $data = SmToDo::find($request->id);
//                $selected->complete_status = $request->complete_status;
                $selected = $data->save();
