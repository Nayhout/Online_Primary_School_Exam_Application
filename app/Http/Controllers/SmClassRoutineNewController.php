...
class SmClassRoutineNewController extends Controller
{
    ...
    public function addNewClassRoutine($class_time_id, $day, $class_id, $section_id)
    {
        ...
        try {
            $assinged_subjects = SmClassRoutineUpdate::select('subject_id')->where('class_id', $class_id)->where('section_id', $section_id)->where('day', $day)->where('created_at', 'LIKE', '' . YearCheck::getYear() . '')->where('school_id',Auth::user()->school_id)->get();//delete % in ''
