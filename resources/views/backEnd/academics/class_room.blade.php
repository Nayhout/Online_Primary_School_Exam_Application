...
<section class="admin-visitor-area up_st_admin_visitor">
    <div class="container-fluid p-0">
        @if(in_array(63, App\GlobalVariable::GlobarModuleLinks()) || Auth::user()->role_id == 1 )
            <div class="offset-lg-3 col-lg-3 text-right mb-20 col-sm-6" style="margin-left: 1150px">
                <a href="{{route('import_room')}}" class="primary-btn small fix-gr-bg">
                    <span class="ti-plus pr-2"></span>
                    @lang('lang.import') @lang('lang.room')
                </a>
            </div>
        @endif
        @if(isset($class_room))
                        ...
                        <div class="white-box">
                            <div class="add-visitor">
                                ...
                                <div class="row mt-40">
                                    <div class="col-lg-12">
                                        <div class="input-effect">
                                                <select class="niceSelect w-100 bb form-control{{ $errors->has('building') ? ' is-invalid' : '' }}" name="building_id" id="building_id">
                                                    <option data-display="@lang('lang.select') @lang('building') *" value="">@lang('lang.select') @lang('building')</option>
                                                    @foreach($building as $row)
                                                        <option value="{{@$row->id}}"
                                                                @if (@$row->id == @$class_room->building_id)
                                                                selected="selected"
                                                                @endif
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <table id="table_id" class="display school-table" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                @foreach($class_rooms->sortByDesc('id') as $class_room)
                                    <?php
//                                            dd($class_room);
                                        $building = \App\SmBuilding::find($class_room->building_id);
                                    ?>
                                <tr>
                                    <td valign="top">{{ @$class_room->room_no}}</td>
                                    <td valign="top">{{ @$building->building_name}}</td>
